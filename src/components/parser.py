...
def parse_leaderboard(html: str) -> List[List[str]]:
    ...
    if next_data_script and next_data_script.string:
        try:
            ...
            _next_data = json.loads(next_data_script.string)
            # Extract leaderboard data from next_data
            # This will depend on the specific structure of the JSON
            logger.debug("Found Next.js embedded JSON: %s", _next_data)
    ...
    if thead:
        ...
        if header_rows:
            ...
            for idx, headers in enumerate(header_texts):
                ...
                score = sum(
                    1
                    for pattern in known_header_patterns
                    if any(pattern in header for header in lower_headers)
                )
                if score > best_score:
                    best_score = score
                    best_idx = idx
            ...
            if best_idx == -1:
                ...
                for idx, headers in enumerate(header_texts):
                    ...
                    unique_count = len(
                        set(h.lower() for h in headers if h and h.strip())
                    )
            ...
            logger.info(
                f"Selected thead header row #{best_idx} with quality score: {best_score if best_score > -1 else best_quality}. Headers: {chosen_headers}"
            )
    ...
    logger.info(
        f"Finished parsing leaderboard. Found {len(result)} rows (including headers)"
    )
    return result
