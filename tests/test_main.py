...
def test_main_orchestration_calls_in_order():
    ...
    with patch("src.main.setup_logger") as mock_setup_logger, patch(
        "src.main.load_config"
    ) as mock_load_config, patch("src.main.fetch_html") as mock_fetch_html, patch(
        "src.main.parse_leaderboard"
    ) as mock_parse_leaderboard, patch(
        "src.main.fetch_html_with_playwright"
    ) as mock_fetch_playwright, patch(
        "src.main.write_to_csv"
        ...
        mock_fetch_playwright.side_effect = lambda url: (
            seq("fetch_html_with_playwright"),
            None,
        )[1]
